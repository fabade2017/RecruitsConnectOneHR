...
export class AdminService {
  ...
  async approveRenewal(renewalId: string, approverId: string) {
    ...
    await this.prisma.organization.update({ where: { id: renewal.organizationId }, data: { isActive: true, status: 'active' } as any }).catch(()=>{});
  ...
  async rejectRenewal(renewalId: string, approverId: string, reason?: string) {
    const renewal = await this.prisma.subscriptionRenewal.findUnique({ where: { id: renewalId } });
    if (!renewal) throw new NotFoundException('Renewal not found');
    if (renewal.status !== 'pending') throw new ConflictException('Renewal not pending');
    return this.prisma.subscriptionRenewal.update({ where: { id: renewalId }, data: { status: 'rejected', approvedBy: approverId } as any });
  }

  async getRenewalReceipt(renewalId: string) {
    ...
    if (!r.receiptNumber) throw new NotFoundException('Receipt not yet generated — awaiting approval');
    return {
