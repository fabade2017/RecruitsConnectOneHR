...
export class OnboardingService {
  ...
  async getChecklists(orgId: string) {
    const workflows = await this.prisma.workflow.findMany({ where: { organizationId: orgId, trigger: 'employee.created' }, take: 5 });
    if (workflows.length) {
      return workflows.map(w => {
        let steps: any[] = [];
        try { steps = JSON.parse(w.steps as any); } catch {}
        return {
          id: w.id,
          name: w.name,
          steps: steps.map((s: any, i: number) => ({ id: String(i+1), title: s.name || s.title, desc: s.type || '', done: false, autoKey: s.type || `step_${i+1}` })),
          workflowId: w.id,
        };
      });
    }
  ...
  private async evaluateAutoSteps(orgId: string, employeeId: string, steps: any[]): Promise<{ steps: any[]; changed: boolean }> {
  ...
  async getOrCreateProgress(orgId: string, employeeId: string) {
    ...
    const currentSteps = JSON.parse(prog.steps as any);
    const evaluated = await this.evaluateAutoSteps(orgId, employeeId, currentSteps);
    if (evaluated.changed) {
      const progress = Math.round((evaluated.steps.filter((s:any)=>s.done).length / evaluated.steps.length)*100);
  ...
  async getByEmployee(orgId: string, employeeId: string) {
    ...
    const steps = JSON.parse(prog.steps as any);
  ...
  async listProgress(orgId: string) {
    ...
    for (const p of progresses) {
      const steps = JSON.parse(p.steps as any);
  ...
  async toggleStep(orgId: string, employeeId: string, stepId: string) {
    const prog = await this.getOrCreateProgress(orgId, employeeId);
    let steps: any[] = JSON.parse(prog.steps as any);
    ...
    if (!steps[idx].done) delete steps[idx].autoVerified;
    const evaluated = await this.evaluateAutoSteps(orgId, employeeId, steps);
    const finalSteps = evaluated.steps;
    const progress = Math.round((finalSteps.filter((s:any)=>s.done).length / finalSteps.length)*100);
  ...
  async updateSteps(orgId: string, employeeId: string, steps: any[]) {
    const prog = await this.getOrCreateProgress(orgId, employeeId);
    const progress = Math.round((steps.filter((s:any)=>s.done).length / steps.length)*100);
    const updated = await this.prisma.onboardingProgress.update({
      where: { employeeId },
