...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    try {
      const token = (client.handshake.auth?.token as string) || (client.handshake.headers.authorization?.replace('Bearer ', '') as string) || (client.handshake.query.token as string);
      if (!token) {
        client.disconnect();
        return;
      }
      const payload: any = jwt.verify(token, process.env.JWT_SECRET || 'change-me-32-chars-minimum-secret-for-dev');
  ...
  async handleMessageSend(@ConnectedSocket() client: Socket, @MessageBody() data: { conversationId: string; content: string; replyToId?: string; messageType?: string; attachments?: any }) {
    ...
    if (!content && !data.attachments) return { error: 'Empty message' };
    ...
    await this.prisma.messageRead.upsert({ where: { messageId_userId: { messageId: msg.id, userId: info.userId } as any }, create: { messageId: msg.id, userId: info.userId }, update: {} });
    ...
    for (const p of participants) {
      if (p.userId === info.userId) continue;
  ...
  async notifyConversationCreated(conversationId: string) {
    const parts = await this.prisma.conversationParticipant.findMany({ where: { conversationId }, select: { userId: true } });
  ...
  async notifyConversationUpdated(conversationId: string, conv: any) {
