...
export class AnalyticsService {
  ...
  async workforceScores(orgId: string, q: any) {
  ...
  async computeLiveScore(orgId: string) {
    ...
    const clockedIn = sessions.filter((s: any) => s.clockInAt).length;
    ...
    for (const r of reviews) {
      try { const kpi = JSON.parse(r.kpi as any); const rating = Array.isArray(kpi) ? (kpi[0]?.rating || 0) : 0; if (rating) { perfSum += (rating / 5) * 100; perfCount++; } } catch {}
    }
  ...
  async dashboard(orgId: string, q: any) {
    ...
    const byArrangement = await this.prisma.employee.groupBy({ by: ['workArrangement'], where: { organizationId: orgId }, _count: true } as any).catch(() => []);
  ...
  async reports(orgId: string, q: any) {
  ...
  async activity(orgId: string, q: any) {
  ...
  async risks(orgId: string) {
    ...
    const overdueReviews = await this.prisma.performanceReview.findMany({ where: { organizationId: orgId }, take: 100 }).then((rs: any[]) => rs.filter((r: any) => !r.reviewDate || new Date(r.reviewDate) < new Date(Date.now() - 90*24*60*60*1000)));
    ...
    return {
      critical: [
        { type: 'expired_certifications', count: expiredCerts.length, employees: expiredCerts.slice(0,3).map((c: any) => c.employeeId), severity: 'critical' },
  ...
  async earlyWarnings(orgId: string, q: any) {
    const employeeId = q.employee_id || q.employeeId;
    if (employeeId) {
      const emp = await this.prisma.employee.findFirst({ where: { id: employeeId, organizationId: orgId } });
      ...
      const lateCount = sessions.filter((s:any) => (s.lateMinutes || 0) > 0).length;
      const absent = sessions.filter((s:any) => !s.clockInAt).length;
      let indicator = 'stable';
      let factors: string[] = [];
      if (lateCount >= 5) { indicator = 'elevated'; factors.push(`Repeated lateness (${lateCount}/30)`); }
      if (absent >= 3) { indicator = 'elevated'; factors.push(`Absence pattern (${absent}/30)`); }
      if (factors.length >= 2) indicator = 'high';
  ...
  async simulate(orgId: string, dto: any) {
    ...
    if (scenario === 'salary_increase' || scenario === 'salaryIncrease') {
      ...
      const totalMonthly = payrolls.reduce((a: number, p: any) => a + Number(p.basicSalary || 0) + Number(p.allowances || 0), 0) || 12500000; // fallback
  ...
  async digitalTwin(orgId: string, q: any) {
