...
function drawPlaceholder(doc: any, label: string, x: number, y: number, w: number, h: number) {
...
function tryEmbed(doc: any, imgPath: string, x: number, y: number, w: number, h: number): boolean {
  try {
...
async function main() {
  ...
  let steps: any[] = []; try { steps = JSON.parse(workflow.steps as any); } catch {}
  const instances = await prisma.workflowInstance.findMany({ where: { workflowId: workflow.id }, orderBy: { createdAt: 'desc' }, include: { approvals: true } as any });
  const employeeIds = [...new Set(instances.map(i => i.entityId))];
  const employees = await prisma.employee.findMany({ where: { id: { in: employeeIds } }, include: { department: true, branch: true, user: true } as any });
  const empMap = new Map(employees.map(e => [e.id, e]));
  const userIds = [...new Set(instances.flatMap(i => (i.approvals as any[]).map(a=>a.approverId).concat(employees.map(e=>e.userId).filter(Boolean))))] as string[];
  ...
  roles.forEach((r, idx) => {
    ...
    doc.fillColor(r.color as any).fontSize(8).font('Helvetica-Bold').text(`${r.role} — ${r.email} / ${r.pass}`, 40, y+6);
    ...
    if (hasLogin) {
      try { doc.image(loginShot, 40, y+54, { width: 255, height: 62, fit: [255, 62] }); doc.rect(40, y+54, 255, 62).stroke('#cbd5e1'); } catch { drawPlaceholder(doc, `Screenshot: Login as ${r.email} → ${r.pages[0].split('→')[1]?.trim() || ''}`, 40, y+54, 255, 62); }
    } else {
    ...
    if (hasOnboard) {
      try { doc.image(onboardShot, 300, y+54, { width: 255, height: 62, fit: [255, 62] }); doc.rect(300, y+54, 255, 62).stroke('#0ea5e9'); } catch {
  ...
  for (const inst of instances) {
    ...
    if (prog) {
      const s = JSON.parse(prog.steps as any);
      if (y > 720) { doc.addPage(); y = 36; }
      doc.fillColor('#7c3aed').fontSize(7).font('Helvetica-Oblique').text(`OnboardingProgress (${prog.progress}%): ${s.map((x:any)=> `${x.title} ${x.done? '✓'+(x.autoVerified?' auto':' manual'): '○'}`).join(' • ')}`, 44, y, { width: 507 });
      y += 14;
    }
    y += 6;
  }
  ...
  y += 14;
  auditLogs.slice(0, 60).forEach((log:any) => {
    if (y > 750) { doc.addPage(); y = 36; }
