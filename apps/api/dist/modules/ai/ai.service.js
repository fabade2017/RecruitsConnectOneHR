...
let AiService = class AiService {
    ...
    async callOpencode(prompt, system) {
    ...
    async ragAnswer(orgId, question, mode, user) {
        ...
        const scored = docs.map(d => {
            ...
            for (const kw of keywords)
                if (t.includes(kw))
                    score += 1;
            if (qLower.includes('leave') && t.includes('leave'))
                score += 3;
            if (qLower.includes('payroll') && t.includes('payroll'))
                score += 3;
            if (qLower.includes('attendance') && t.includes('attendance'))
                score += 3;
            if (qLower.includes('policy') && d.type === 'policy')
                score += 2;
            return { ...d, score };
        }).sort((a, b) => b.score - a.score).slice(0, 5);
        const top = scored.filter(s => s.score > 0);
        let citations = top.map(t => ({ policy_id: t.id, title: t.title, type: t.type }));
        ...
        const tryOrder = [provider, 'groq', 'ollama', 'opencode', 'openai', 'mock'].filter((v, i, a) => a.indexOf(v) === i);
        for (const p of tryOrder) {
            if (p === 'mock')
                break;
            let res = null;
            if (p === 'groq')
                res = await this.callGroq(prompt, systemPrompt);
            else if (p === 'ollama')
                res = await this.callOllama(prompt, systemPrompt);
            else if (p === 'openai')
                res = await this.callOpenAI(prompt, systemPrompt);
            else if (p === 'opencode')
                res = await this.callOpencode(prompt, systemPrompt);
            if (res) {
                answer = (p !== provider ? `[fallback ${p}] ` : '') + res;
                llmUsed = p;
                break;
            }
        }
        if (!answer) {
            ...
            if (!answer.startsWith(prefix.trim()))
