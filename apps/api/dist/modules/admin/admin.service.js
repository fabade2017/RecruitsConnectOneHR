...
let AdminService = class AdminService {
    ...
    async assignModulesToPlan(planId, modules) {
        ...
        // Remove existing modules not in new list, add new ones
        await this.prisma.planModule.deleteMany({ where: { planId, moduleKey: { notIn: modules } } });
        for (const mod of modules) {
            await this.prisma.planModule.upsert({
                where: { planId_moduleKey: { planId, moduleKey: mod } },
                create: { planId, moduleKey: mod, enabled: true },
                update: { enabled: true },
            });
        }
