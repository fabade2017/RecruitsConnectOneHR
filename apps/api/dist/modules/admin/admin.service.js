...
let AdminService = class AdminService {
    ...
    async createRole(dto, user) {
        ...
        if (exists)
            throw new common_1.ConflictException('Role slug already exists');
    ...
    async upsertModuleCatalog(dto) {
        const key = dto.key?.toLowerCase().trim();
        if (!key)
            throw new common_1.ConflictException('Module key required');
        return this.prisma.moduleCatalog.upsert({
            where: { key },
            create: {
                key,
                name: dto.name || key.replace(/_/g, ' ').replace(/\b\w/g, (s) => s.toUpperCase()),
                description: dto.description,
    ...
    async updateModuleCatalog(key, dto) {
    ...
    async createGroup(dto, user) {
        if (user.role !== 'super_admin')
            throw new common_1.ForbiddenException('Only super_admin can create company groups');
    ...
    async assignOrganizationToGroup(groupId, orgId) {
    ...
    async groupHierarchy(groupId) {
        ...
        return {
            ...
            totalEmployees: group.organizations.reduce((sum, org) => sum + org._count.employees, 0),
            totalBranches: group.organizations.reduce((sum, org) => sum + org._count.branches, 0),
        };
    }
    ...
    async assignModulesToPlan(planId, modules) {
        const plan = await this.prisma.subscriptionPlan.findUnique({ where: { id: planId } });
        if (!plan)
            throw new common_1.NotFoundException('Plan not found');
        // Normalize to {key, price}
        const normalized = modules.map((m) => typeof m === 'string' ? { key: m, price: undefined } : { key: m.key || m.moduleKey, price: m.price });
    ...
    async setModulePrice(planId, moduleKey, price) {
    ...
    async assignSubscription(dto) {
        ...
        if (!organizationId && !companyGroupId)
            throw new common_1.ForbiddenException('Must provide organizationId or companyGroupId');
        const plan = await this.prisma.subscriptionPlan.findUnique({ where: { id: planId } });
        if (!plan)
            throw new common_1.NotFoundException('Plan not found');
    ...
    async checkModuleAccess(organizationId, moduleKey) {
