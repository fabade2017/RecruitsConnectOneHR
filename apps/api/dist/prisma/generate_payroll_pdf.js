...
async function main() {
    ...
    const payrolls = await prisma.payrollMerged.findMany({ where: { id: { in: payrollIds } }, include: { employee: true } });
    const payrollMap = new Map(payrolls.map(p => [p.id, p]));
    const userIds = [...new Set(instances.flatMap(i => i.approvals.map((a) => a.approverId).concat([i.organizationId])))];
    ...
    let condition = {};
    try {
        condition = JSON.parse(workflow.condition);
    }
    catch { }
    let escalation = {};
    try {
        escalation = JSON.parse(workflow.escalation);
    }
    catch { }
    ...
    for (const inst of instances) {
        ...
        for (let idx = 0; idx < approvals.length; idx++) {
            ...
            const audit = auditLogs.find((log) => log.entityId === a.id || log.newValue?.includes(a.approverId));
